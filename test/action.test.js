  ...
  describe("Entrypoint execution", () => {
    ...
    test("all inputs provided", () => {
      const testInputs = {
        ...
        max_secret: { value: "10" },
        num_cpu: { value: "1" },
        disable_git_scan: { value: "true" },
        ignore_git_history_baseline: { value: "true" },
        from_commit: { value: "test-from-commit" },
        to_commit: { value: "test-to-commit" },
    ...
    test("empty optional inputs", () => {
      ...
      expect(cliArgs).toEqual(
        `--project-key ${testInputs.project_key.value} secrets scan --path ${testInputs.path.value} --format table,json --output orca_results/ --console-output=table`,
      );
    });
